        ...
        static void Main(string[] args)
        {
            while (true)
            {
                try
                {
                    ...
                    client.StartReceiving(new UpdateHandler(new UserService(), new ToDoService(maxTasks, maxLengthNameTask)));
        ...
        private static int ParseAndValidateInt(string? str, uint min, uint max)
        {
            ...
            if (parseFlag)
                if (parseInt <= max && parseInt >= min)
                    ...
                    throw new UserException($"Вы ввели \"{parseInt}\", число выходит за пределы указанного диапазона [{min}:{max}]!");
